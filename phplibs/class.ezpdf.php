...
function Cezpdf($paper='a4',$orientation='portrait'){
	...
	if (!is_array($paper)){
		switch (strtoupper($paper)){
			...
			case '11X17': {$size = array(0,0,792.00,1224.00); break;}
...
function ezTable(&$data,$cols='',$title='',$options=''){
  ...
  // jbtemp $middle calculation was corrected by Jeff Buck... $options['gap']
  // was not being considered when calculating middle, even though it gets
  // included into the table width one more time than it actually shows...
  // This actually should have been corrected elsewhere, but I'm too lazy to
  // do it there when it seems to work (mostly) from here..
  $middle = (($this->ez['pageWidth']-$this->ez['rightMargin'])/2)+(($this->ez['leftMargin'])/2) + ($options['gap']/2);
...
//        $this->reopenObject($textObjectId);
    ...
    ///////////////////////////////////////////////////////////////
	// $options['splitRows']==0 can cause an infinite loop on large
	// tables when they hit the first page break. This marker is to 
	// help find this later so I can fix this.. for now though, 
	// I'll just use splitrows=1 from other functions to make sure
	// this bug isn't tripped on.
	// jbtemp jbmarker jb findme find me fixme fix me
    ///////////////////////////////////////////////////////////////
    //$options['splitRows']=1;
    if ($options['splitRows']==0){
...
function ez_umc_header_start($text="",$fontsize="",$textoptions="",$redo_header=0) {
	...
	if ($this->ez_umc_letterhead_defined!="1") {
		...
		$header_height=($this->ez['pageHeight'] - ($height=40) - 37);
		...
		$this->addPngFromFile("/data/web/pipeline/global/timesheet/umclogo_wbv2.png",$this->ez['leftMargin'],$header_height,120,$height);
	...
	$this->ezSetY($this->ez['pageHeight'] - $this->ez['topMargin']);
	}
...
function ezTableDumpPrint($firstrowheaders=0,$array_options="") {
	...
	if ($array_options['title']!="") $title=$array_options['title'];
	...
	if ($array_options!="") {
		...
		$this->ezTable($this->tabledata,$headings,$title,$array_options);
		} else {
		$this->ezTable($this->tabledata,$headings,$title,$base_array);
		}
...
///////////////////////////////////////////////////
function ezTableDump($query,$title="",$options="") {
	$res=@mysql_query($query);
	$data=$this->mysql_to_array($res);
	$maxwidth=$this->ez['pageWidth'] - $this->ez['leftMargin'] - $this->ez['rightMargin'];
	$defaults=array('maxWidth'=>"$maxwidth");
	if ($options=="") $options_array=$defaults;
	else $options_array=array_merge($defaults,$options);
	//$this->ezTable($data,"","$title",array('maxWidth'=>"$maxwidth"));
	$this->ezTable($data,"","$title",$options_array);
	//$this->ezTable($data,"","$title");
	}
